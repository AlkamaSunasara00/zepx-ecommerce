...
function Products() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const res = id
                    ? await axios.get(`http://localhost:4800/getproductsbycategoryid/${id}`)
                    : await axios.get(`http://localhost:4800/getproducts`);

                setFetchProducts(res.data);
                setFilteredProducts(res.data);
                setShuffledProducts(shuffleArray(res.data));
            } catch (error) {
    ...
    useEffect(() => {
        ...
        setFilteredProducts(results);
        setShuffledProducts(shuffleArray(results)); // Shuffle filtered products
    }, [searchTerm, fetchProducts]);
                        ...
                        return ( 
                            <NavLink to={`/item/${item.product_id}`} key={item.product_id}>
                                <div className='products-card'>
                                    ...
                                    <div className='products-price'>
                                        <p className='products-original-price'>₹{item.price}</p>
                                        <p className='products-discount-price'>
